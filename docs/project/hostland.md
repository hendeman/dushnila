...
После серверной части workflow проверяет главную страницу, `robots.txt` и `sitemap.xml` через публичный HTTPS-домен. Миграции для автоматического деплоя должны быть обратно совместимыми: удаление или переименование используемых полей и таблиц выполняется отдельным согласованным обновлением.
